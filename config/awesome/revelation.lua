...
function revelation(num, s)
  local scr = s or capi.mouse.screen
  local t = capi.screen[scr]:tags()
  local ot = awful.tag.selectedlist(scr)
  local oldlayout = awful.layout.get( scr )

  if num ~= 1 then  awful.tag.viewmore(t, scr) end
  awful.layout.set(awful.layout.suit.fair.horizontal)

  local function restore()
    awful.layout.set(oldlayout)
    if num ~= 1 then
        awful.tag.viewmore(ot, scr)
    end
