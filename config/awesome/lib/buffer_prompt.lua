...
-- @param args A table with optional arguments: fg_cursor, bg_cursor, ul_cursor, prompt, text, selectall, font, autoexec, execaftereach.
...
function run(args, textbox, exe_callback, completion_callback, history_path, history_max, done_callback)
    ...
    local ncomp = 1
    -- Variable for running completion function after the key press
    local compl_after = false
    ...
    function (modifiers, key, event)
        ...
        if mod.Control then
            ...
            if completion_callback then
                if key == "Tab" or (key == "ISO_Left_Tab" or compl_after) then
                    if key == "ISO_Left_Tab" or compl_after then
                    ...
                    if matches and #matches == 1
                        and ( args.autoexec or args.exec_aea ) then
                        exec()
                    end
                    return true
                else
            ...
            elseif key == "Down" then
               if history_index < history_items(history_path) then
                ...
                -- If only one match found and exec_ea flag
                if args.execa_ea or args.autoexec_last then
                    compl_after = true
                end
