...
class CustomApplications(Applications):
	...
	def app_vim(self, c):
		...
		if c.mode in arg: # mode 1 will open files in tabs, 2 - in splitted windows
			return tup('vim', arg[c.mode], c.file.path)
	...
	@depends_on('feh')
	def app_feh(self, c):
		arg = {1: '--bg-scale', 2: '--bg-tile', 3: '--bg-center', 4: '--bg-fill'}
